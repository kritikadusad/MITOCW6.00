...
def get_word_score(word, n):
    ...
    if n > len(word):
        for c in range(0, len(word)):
            score += SCRABBLE_LETTER_VALUES[word[c]]
        return score * len(word)
    else:
        for c in range(0, len(word)):   
            score += SCRABBLE_LETTER_VALUES[word[c]]
            # print score
            # print SCRABBLE_LETTER_VALUES[word[c]]
...
def is_valid_word(word, hand, word_list):
    ...
    """
    # TO DO...
    check_hand = dict(hand)
    if word in word_list:
        for i in range(0, len(word)):
            if word[i] in check_hand:
                check_hand[word[i]] -= 1
                if check_hand[word[i]] == 0:
                    del check_hand[word[i]]
            else: 
                return False
        return True
    else: 
...
# hand = {'a': 1, 's':1, 'w':1, 'f':1, 't':2, 'o':1}

# a = play_hand(hand, word_list)
# print a
